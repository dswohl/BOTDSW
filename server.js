...
app.use('/ios', express.static('ios')); // Nueva línea para servir la carpeta ios
...
client.on('message', async (msg) => {
    console.log('Evento de mensaje recibido'); // Nuevo log
    
    if (msg.fromMe) {
        console.log('Mensaje propio ignorado'); // Nuevo log
        return;
    }
    ...
    try {
        console.log('Intentando leer responses.json'); // Nuevo log
        const responses = JSON.parse(fs.readFileSync('responses.json', 'utf8'));
        console.log('Respuestas cargadas:', responses);
        
        let foundMatch = false;
        console.log('Buscando coincidencias...'); // Nuevo log
        
        for (const [keywords, response] of Object.entries(responses)) {
            console.log(`Evaluando keyword: ${keywords}`); // Nuevo log
            const keywordArray = keywords.toLowerCase().split(',').map(k => k.trim());
            console.log('Keywords procesadas:', keywordArray); // Nuevo log
            
            if (keywordArray.some(keyword => text.includes(keyword))) {
                console.log(`Coincidencia encontrada con: ${keywords}`); // Nuevo log
        ...
        if (!foundMatch) {
            console.log('No se encontraron coincidencias, usando respuesta default'); // Nuevo log
        ...
        console.error('Stack:', err.stack); // Nuevo log
    }
});
